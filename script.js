...
// ============ 1. خريطة الدول واللغات (Detection Map) ============
...
// ============ 2. دالة الكشف التلقائي عن اللغة ============
async function detectLanguage() {
    // محاولة جلب اللغة من الرابط (URL) أولاً
    const urlLang = new URLSearchParams(window.location.search).get('lang');
    
    if (urlLang && translations[urlLang]) {
        setLang(urlLang);
        updateDropdownUI(urlLang);
        return;
    }

    // محاولة الكشف عن اللغة بناءً على موقع المستخدم (IP)
    try {
        const response = await fetch('https://ipapi.co/json/');
        const data = await response.json();
        const detectedLang = countryLangMap[data.country_code] || 'tr';
        setLang(detectedLang);
        updateDropdownUI(detectedLang);
    } catch (error) {
        // في حال فشل الاتصال، نعتمد اللغة الافتراضية
        setLang('tr');
        updateDropdownUI('tr');
    }
}

// ============ 3. دالة التحكم في القائمة المنسدلة المخصصة ============
function initCustomSelect() {
    ...
    if (customSelect) {
        customSelect.addEventListener('click', function (e) {
            ...
            e.stopPropagation(); // منع إغلاق القائمة فوراً عند الضغط عليها
        });
    }
...
// ============ 4. دالة اختيار اللغة من القائمة (Trigger) ============
function selectLang(lang, flagUrl, text) {
    // تحديث واجهة القائمة المنسدلة (الصورة والنص)
    ...
    // استدعاء دالة تغيير اللغة الأساسية
    setLang(lang);
}

// ============ 5. دالة تحديث واجهة القائمة عند التغيير التلقائي ============
...
// ============ 6. دالة الترجمة وتغيير المحتوى (Core Function) ============
function setLang(lang) {
    // التأكد من أن اللغة موجودة في ملف الترجمات
    ...
    // تحديث اتجاه الصفحة ولغتها الأساسية
    document.documentElement.lang = lang;
    document.documentElement.dir = isRTL ? 'rtl' : 'ltr';

    // ترجمة جميع العناصر التي تحمل الوسم data-i18n
    ...
    if (metaKeywords && t['meta.kw']) metaKeywords.content = t['meta.kw'];

    // تحديث روابط التواصل الاجتماعي (Open Graph)
    const ogTitle = document.getElementById('og-title');
    if (ogTitle && t['meta.title']) ogTitle.content = t['meta.title'];

    const ogDesc = document.getElementById('og-desc');
    if (ogDesc && t['meta.desc']) ogDesc.content = t['meta.desc'];

    // تحديث النصوص المساعدة (Placeholders) في النماذج
    const placeholders = {
        tr: ['Adınızı girin', 'E-posta adresiniz', 'Mesajınızı yazın...'],
        en: ['Enter your name', 'Your email address', 'Write your message...'],
        ar: ['أدخل اسمك', 'بريدك الإلكتروني', 'اكتب رسالتك...'],
        ru: ['Введите ваше имя', 'Ваш email адрес', 'Напишите ваше сообщение...'],
        fr: ['Entrez votre nom', 'Votre adresse e-mail', 'Écrivez votre message...']
    };

    const inputs = document.querySelectorAll('.contact-form input, .contact-form textarea');
    if (placeholders[lang] && inputs.length >= 3) {
        if (inputs[0]) inputs[0].placeholder = placeholders[lang][0];
        if (inputs[1]) inputs[1].placeholder = placeholders[lang][1];
        if (inputs[2]) inputs[2].placeholder = placeholders[lang][2];
    }
}

// ============ 7. التنفيذ عند تحميل الصفحة ============
...
// ============ FORM SUBMIT ============
function submitForm() {
  const msgs = { tr: 'Mesajınız gönderildi! Teşekkürler.', en: 'Message sent! Thank you.', ar: 'تم إرسال رسالتك! شكراً.', ru: 'Сообщение отправлено! Спасибо.', fr: 'Message envoyé ! Merci.' };
...
function initHeroSlider() {
    ...
    if (slides.length === 0) return; // تأمين الكود في حال عدم وجود صور

    let currentSlide = 0;
    const slideInterval = 4000; // 4 ثوانٍ لكل صورة
